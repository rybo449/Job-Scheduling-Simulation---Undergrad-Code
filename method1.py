...
def print_output():
	...
	for i in xrange(blocks):
		...
		print
	print "Gang Queue Status: ",gang_dict
	print
	...
	for i in xrange(blocks):
		for j in xrange(processors):
			...
			for k in xrange(len(processor_dict[i][j])):
				if processor_dict[i][j][k]==0:
					#count_processsor_dict=count_processor_dict+1
					#print "Count=",count_processor_dict
					'''if len(gang_dict)>0:
						w=gang_dict.popleft()
						q=gang_job_length.popleft()
						print "Gang Job ",w," entered in Queue"
						print
						add_to_local_queue(w*q,i)
						time.sleep(5)'''
						
					if len(local_dict[i])>0:
						processor_dict[i][j][k]=local_dict[i].popleft()
					else:
						count_empty_processors+=1

			if(len(gang_dict)>0):
				if count_empty_processors>=gang_job_length[0]:
					x=gang_job_length.popleft()
					y=gang_dict.popleft()
					print "Gang Job %d Has Entered found a place in the Local Queue at Time %d"%(y,clock_ticks)
					print
					for l in xrange(10):
						if x>0:
							x=x-1
							if processor_dict[i][j][l]==0:
								processor_dict[i][j][l]=y
								time.sleep(2)
			...
			if count_empty_processors>=gang_job_length[0]:
				...
				for l in xrange(10):
					if x>0:
						...
						if processor_dict[i][j][l]==0:
							processor_dict[i][j][l]=y
							print "Gang Job Has Entered found a place in the Local Queue at Time 0"
							print
							time.sleep(2)
